...
/*try {
    ...
    const condition = (el) => el.userId === userLikeInfoToAdd.userId;
    const foundUser = postInstance.usersEngagement.find(condition);

    if (foundUser) {
      foundUser.userStatus = likeStatus;
    }

    /!* const postInstLikeStat = (postInstance.usersEngagement.find(
            (el) => el.userId === userLikeInfoToAdd.userId,
          )!.userStatus = likeStatus);*!/

    //console.log(postInstLikeStat, ' postInstLikeStat');
